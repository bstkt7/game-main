...
const StartScreen: React.FC<StartScreenProps> = ({ onStart, isMobile, isPortrait }) => {
  ...
  // Эффект для visualViewport height
  useLayoutEffect(() => {
    const updateHeight = () => {
  ...
  // Эффект для отслеживания изменений Fullscreen
  useEffect(() => {
  ...
  // Эффект для отслеживания ориентации устройства (Параллакс)
  useEffect(() => {
  ...
  // Расчет смещения для параллакса
  ...
  const overlayStyle: React.CSSProperties = {
    ...
    padding: isMobile
      ? `0 calc(10px + env(safe-area-inset-right)) 0 calc(10px + env(safe-area-inset-left))`
      : '32px',
    gap: isMobile ? '0' : '48px',
    zIndex: 10,
    overflow: 'hidden',
  ...
  const logoStyle: React.CSSProperties = {
    width: isMobile ? (isPortrait ? '60%' : '80%') : '0',
    maxWidth: isMobile && !isPortrait ? '200px' : '100%',
    height: 'auto',
  };
  ...
  const rotationOverlayStyle: React.CSSProperties = {
    ...
    padding: '15px',
    boxSizing: 'border-box',
    display: isMobile && isPortrait ? 'block' : 'none',
  };
  ...
  const handleMouseLeave = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (isMobile) {
      if (
        e.currentTarget.style.backgroundImage !== fullscreenButtonStyle.backgroundImage &&
        e.currentTarget.style.transform === 'scale(0.97)'
      ) {
        e.currentTarget.style.backgroundImage = 'url(/assets/gvozd/start_button.png)';
        e.currentTarget.style.transform = 'scale(1)';
  ...
  const exitFullscreen = () => {
    ...
    try {
      ...
      console.error('Exit FS failed:', e);
    }
  };
  ...
  const renderMobileLandscape = () => {
    const baseStyle = hornetLandscapeImageStyle;
    const useParallax = isMobile && gyroscopeActive;
        ...
        <div style={rightColumnLandscapeStyle}>
          <div style={instructionsLandscapeBoxStyle}>
            <div style={disclaimerStyle}>
              ККШИ попала в беду — таинственные шершни проникли в окрестности школы и рассыпали уставы, документы,
              учебные материалы по всей территории школы-интерната. Некоторые из кадетских символов украдены.
              <br />
              <br />
              Кадет Пчёла — лучший кадет, и именно он отправляется в путь, чтобы вернуть порядок, собрать уставы, найти
              потерянные шевроны, спасти кадетов, и в финале — отстоять честь школы в поединке с Шершнем-Командиром.
            </div>
          </div>
  ...
  const renderMobilePortrait = () => {
    const baseStyle = hornetImageStyle;
    const useParallax = isMobile && gyroscopeActive;
    ...
    return (
      <div style={mobilePortraitContainerStyle}>
        ...
        <div style={rotationOverlayStyle}>
          ...
          <div style={rotateTextStyle}>Пожалуйста, переверните устройство</div>
        </div>
      </div>
    );
  };
    ...
    <>
      ...
      <div style={rightColumnStyle}>
        <button
          ...
          onMouseOver={handleDesktopMouseOver}
          onMouseOut={handleDesktopMouseOut}
          onMouseDown={handleMouseDown}
          onMouseUp={handleMouseUp}
        >
          СТАРТ!
        ...
        <div style={instructionsBoxDesktopStyle}>
          <div style={disclaimerStyle}>
            ККШИ попала в беду — таинственные шершни проникли в окрестности школы и рассыпали уставы, документы, учебные
            материалы по всей территории школы-интерната. Некоторые из кадетских символов украдены.
            <br />
            <br />
            Кадет Пчёла — лучший кадет, и именно он отправляется в путь, чтобы вернуть порядок, собрать уставы, найти
            потерянные шевроны, спасти кадетов, и в финале — отстоять честь школы в поединке с Шершнем-Командиром.
  ...
  return (
    <div style={overlayStyle} ref={overlayRef}>
      ...
      {isMobile ? (isPortrait ? renderMobilePortrait() : renderMobileLandscape()) : renderDesktop()}
    </div>
  );
};
